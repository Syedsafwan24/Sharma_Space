...
// Utility to get the correct base URL for API calls
export const getBaseUrl = () => {
	// In production, use environment variables or fallback to production domain
	...
	return 'https://www.sharmaspace.in';
};
...
export async function fetchCompanyStats() {
	try {
		const baseUrl = getBaseUrl();
		const response = await fetch(`${baseUrl}/api/company-stats`);
...
export async function fetchPartnerBrands() {
	try {
		const baseUrl = getBaseUrl();
		const response = await fetch(`${baseUrl}/api/partner-brands`);
